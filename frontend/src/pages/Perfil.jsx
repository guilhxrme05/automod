...
// --- Ícones para a UI (Componentes SVG) ---
...
// <<< NOVO COMPONENTE: MODAL DE RASTREIO >>>
const RastreioModal = ({ pedidoId, onClose }) => {
    ...
    useEffect(() => {
        const fetchRastreio = async () => {
            setLoading(true);
            try {
                const response = await fetch(`http://localhost:3001/api/pedidos/${pedidoId}/rastreio`);
                if (!response.ok) throw new Error("Não foi possível obter os dados de rastreio.");
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content" onClick={e => e.stopPropagation()}>
                ...
                <h2>Rastreio do Pedido #{String(pedidoId).padStart(5, '0')}</h2>
                {loading ? <p>A carregar estado...</p> : (
                    <div className="rastreio-list">
                        {produtos.map(produto => (
                            <div key={produto.id} className="rastreio-item">
                                <span className="rastreio-bloco">Bloco {produto.bloco_numero}</span>
                                <span className={`status-badge status-${produto.status.replace(/\s+/g, '-').toLowerCase()}`}>{produto.status}</span>
                            </div>
                        ))}
                    </div>
                )}
...
const Perfil = () => {
  ...
  return (
    // <<< ATUALIZAÇÃO 1: O return agora está envolvido por um Fragmento >>>
    // Isto permite que o modal exista "por cima" de todo o resto da página
    <>
      {selectedOrderForTracking && (
          <RastreioModal pedidoId={selectedOrderForTracking} onClose={() => setSelectedOrderForTracking(null)} />
      )}
        ...
        <main className="profile-content">
          ...
          {activeTab === 'pedidos' && (
            <section id="pedidos">
              ...
              <p className="section-description">Acompanhe o estado dos seus carros personalizados. Clique num pedido para ver o detalhe.</p>
              <div className="orders-table-container">
                <table className="orders-table">
                  ...
                  <tbody>
                    ...
                    {!loading.orders && orders.length === 0 && <tr><td colSpan="5">Ainda não tem pedidos finalizados.</td></tr>}
                    {orders.map(order => (
                      // <<< ATUALIZAÇÃO 2: A linha da tabela agora é clicável >>>
                      // Apenas para pedidos que não estão no carrinho
                      <tr key={order.id} 
                          onClick={() => order.status !== 'No carrinho' && setSelectedOrderForTracking(order.id)}
                          className={order.status !== 'No carrinho' ? 'trackable' : ''}>
